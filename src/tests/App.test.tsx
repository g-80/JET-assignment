...
function makeRestaurantDto(overrides: Record<string, unknown> = {}) {
  return {
    ...
    address: {
      ...
      city: 'London',
      postalCode: 'W1A 1AA',
      location: { type: 'Point', coordinates: [-0.1, 51.5] as [number, number] },
    },
    ...overrides,
  }
}
...
describe('Initial render', () => {
  ...
  it('renders the "Display restaurants" button', () => {
    render(<App />)
    expect(screen.getByRole('button', { name: /Display restaurants/i })).toBeInTheDocument()
  })
...
describe('Successful fetch', () => {
  ...
  it('displays restaurant cards after a successful fetch', async () => {
    ...
    render(<App />)
    fireEvent.click(screen.getByRole('button', { name: /Display restaurants/i }))
  ...
  it('displays the full address on the card', async () => {
    ...
    await waitFor(() =>
      expect(screen.getByText(/1 Test Street, London, W1A 1AA/i)).toBeInTheDocument()
    )
  })
  ...
  it(`slices results to at most ${NUM_RESULTS_DISPLAYED} cards`, async () => {
    ...
    await waitFor(() => {
      expect(screen.getAllByRole('listitem')).toHaveLength(NUM_RESULTS_DISPLAYED)
    })
  })
...
describe('Error handling', () => {
  ...
  it('shows an error message when the API returns a non-OK response', async () => {
    ...
    await waitFor(() =>
      expect(screen.getByText(/The API returned an error/i)).toBeInTheDocument()
    )
  })
...
describe('Loading state', () => {
  ...
  it('hides the button while loading', async () => {
    ...
    await waitFor(() =>
      expect(screen.queryByRole('button', { name: /Display restaurants/i })).toBeNull()
    )
  })
